...
void ofxCvOpticalFlowLK::draw( float x, float y, float w, float h ) {
    ...
    float scalex = 0.0f;
    float scaley = 0.0f;
    if( captureWidth != 0 ) { scalex = w/captureWidth; } else { scalex = 1.0f; }
    if( captureHeight != 0 ) { scaley = h/captureHeight; } else { scaley = 1.0f; }
...
//	float scaley = (float) height/captureHeight;
	...
	int _x, _y, _dx, _dy;
	for ( _y = 0; _y < captureHeight; _y+=captureRowsStep ){
		for ( _x = 0; _x < captureWidth; _x+=captureColsStep ){
			
				//			dx = (int)cvGetReal2D( vel_x, y, x );
				//			dy = (int)cvGetReal2D( vel_y, y, x );
				//			
			ofPoint vec = flowAtPoint(_x, _y);
			ofLine(_x,_y,_x+vec.x,_y+vec.y);
	...
	//--------------------------------------------------------------------------------
void ofxCvOpticalFlowLK::setAnchorPercent( float xPct, float yPct ){
    anchor.x = xPct;
    anchor.y = yPct;
    bAnchorIsPct = true;
}
	...
	//--------------------------------------------------------------------------------
void ofxCvOpticalFlowLK::setAnchorPoint( int x, int y ){
    anchor.x = x;
    anchor.y = y;
    bAnchorIsPct = false;
}
	...
	//--------------------------------------------------------------------------------
void ofxCvOpticalFlowLK::resetAnchor(){
    anchor.set(0,0);
    bAnchorIsPct = false;
}
